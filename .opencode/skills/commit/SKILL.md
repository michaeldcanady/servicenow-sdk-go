...
## Step 2 — Decide on the grouping, then make it atomic

Before staging anything, look at the whole working tree and decide how the
changes should **group** — one commit or several. This is a deliberate
decision, not something to stumble into: ask whether each hunk/file belongs
to the same unit of meaning as the next, or should be its own commit.

A commit is a unit of **meaning**, not a unit of time. Group work so each
commit is one logical change: one bug, one feature, one refactor, one fix.
Rules of thumb:

- **Group together** changes that share a purpose and only make sense as a
  whole — e.g. a change plus its tests, a cross-cutting rename, or one
  feature's scattered edits across several files. Splitting these into
  per-file commits would break the build and leave each commit meaningless.
- **Split apart** changes that are independent, even if you made them at the
  same time. If a change spans multiple unrelated concerns (e.g. a fix plus
  a drive-by rename), split them — stage different files (or different hunks
  with `git add -p`) into separate commits.
- When in doubt, split: separate commits are easy to review, revert, or
  reorder independently, and can always be squashed later. Fusing them later
  is harder.
...
part — don't force one message to cover everything. State the grouping you
chose (single vs. multiple, and what goes in each) so the decision is
visible.
...
- Bundling unrelated fixes into one commit so a release note reads as one
  feature.
- Zero-commit-per-file reflex — splitting a single logical change into many
  per-file commits fragments the meaning and can break the build between
  commits; a change's tests belong in the same commit as the change.
- Not considering the grouping at all and committing whatever happened to be
  staged, so the PR's commit history doesn't reflect the actual units of
  work.
