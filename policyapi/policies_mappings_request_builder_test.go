...
func TestPoliciesMappingsRequestBuilder_Post(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			setup: func(adapter *mocking.MockRequestAdapter) {
				adapter.On("Send", mock.Anything, mock.Anything, mock.Anything, mock.Anything).Return(internal.NewBaseServiceNowItemResponse[*PoliciesMapping](CreatePoliciesMappingsInputFromDiscriminatorValue), nil)
			},
			wantErr: false,
		},
		{
