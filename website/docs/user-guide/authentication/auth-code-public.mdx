...
```mermaid
flowchart TD
    A([App code]):::app --> B(NewPublicAuthorizationCodeProvider<br/>client ID only — no secret):::sdk
    B --> C(Public client<br/>secured with PKCE S256):::sdk

    subgraph acquire ["Getting the first token (interactive)"]
        H(Start local redirect listener<br/>on port 5001):::sdk --> J[[Browser opens the authorization URL<br/>with the PKCE challenge]]:::external
        J --> K[[User signs in<br/>and approves]]:::external
        K --> L(Redirect delivers the<br/>authorization code):::sdk
        L --> M(Exchange code for tokens<br/>with the PKCE verifier):::sdk
...
<GoSnippet language="go" src={authGo} region="auth_code_public" />
