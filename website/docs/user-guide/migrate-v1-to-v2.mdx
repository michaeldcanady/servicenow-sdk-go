...
<GoExample language="go" title="v2" files={{ attachmentsGo: attachmentsGo }} template={`{{attachmentsGo:attachment_create_guide}}

{{attachmentsGo:attachment_download_guide}}`} />

See [Attachments](attachments/index.mdx) for the full upload and download
guides.
...
[`core.PageIterator`](pagination.mdx), which follows `Link` headers across
pages automatically:
...
<GoSnippet language="go" title="v2 — iterate all pages" src={paginationGo} region="pagination_table_manual" />

<GoSnippet language="go" title="v2 — item by item" src={paginationGo} region="pagination_item_by_item" />

The iterator also works for attachments and other collection endpoints —
see [Pagination](pagination.mdx) for the full API including reverse
...
<GoSnippet language="go" title="v2" src={queryGo} region="query_basic" />

<GoSnippet language="go" title="v2 — pass to table API" src={queryGo} region="query_table_api" />

See [Query builder](query-builder.mdx) for the full condition API.
...
<GoExample language="go" title="v2" files={{ authGo: authGo }} template={`{{authGo:auth_basic}}
...
<GoExample language="go" title="v2 — ROPC" files={{ authGo: authGo }} template={`{{authGo:auth_ropc}}`} />

If you implemented v1's `Credential` interface yourself, implement Kiota's
`authentication.AuthenticationProvider` instead — see
[Authentication](authentication/index.md) for the full provider reference.
