...
description: >-
  ...
  with the full ADR catalog one click deeper for the technical whys and hows.
---
...
to its **Architecture Decision Record (ADR)** in the [ADR
catalog](adrs/index.md), which is the primary source when you want the deeper
technical why and how, the alternatives considered, and the trade-offs
accepted.
...
1. **Write the ADR** in the [ADR catalog](adrs/index.md) under
   `website/docs/contributing/adrs/` — context, decision, consequences,
   alternatives considered. The ADR is the primary source and the artifact
   reviewers evaluate; its index page explains numbering and registration.
