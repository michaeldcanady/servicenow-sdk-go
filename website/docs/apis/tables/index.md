...
- [**List Records**](list.mdx): Retrieve multiple records from a table with support for filtering, sorting, and pagination.
- [**Get Record**](get.mdx): Retrieve a single record by its `sys_id`.
- [**Create Record**](create.mdx): Insert a new record into a table.
- [**Update Record**](update.mdx): Modify an existing record by its `sys_id`.
- [**Delete Record**](delete.mdx): Remove a record from a table by its `sys_id`.
