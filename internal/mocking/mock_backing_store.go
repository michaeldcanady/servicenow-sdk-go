...
var _ store.BackingStore = (*MockBackingStore)(nil)
...
func NewMockBackingStore() *MockBackingStore {
	return &MockBackingStore{Mock: mock.Mock{}}
}
...
func (bS *MockBackingStore) SubscribeWithId(callback store.BackingStoreSubscriber, subscriptionId string) error { //nolint:stylecheck
	args := bS.Called(callback, subscriptionId)
	return args.Error(0)
}

// Unsubscribe Removes a subscription from the store based on its subscription id.
func (bS *MockBackingStore) Unsubscribe(subscriptionId string) error { //nolint:stylecheck
