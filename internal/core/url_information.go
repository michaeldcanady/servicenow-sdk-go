...
func (uI *UrlInformation) validateURLTemplate() error {
	...
	if !strings.Contains(strings.ToLower(uI.UrlTemplate), baseURLTemplatePlaceholder) {
...
func (uI *UrlInformation) validatePathParams() error {
	...
	_, baseurlExists := uI.PathParameters[baseURLkey]
	if !baseurlExists {
...
func (uI *UrlInformation) checkBaseURLRequirement() error {
	_, baseurlExists := uI.PathParameters[baseURLkey]
	if !baseurlExists && strings.Contains(strings.ToLower(uI.UrlTemplate), baseURLTemplatePlaceholder) {
