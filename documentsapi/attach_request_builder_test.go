...
func TestAttachRequestBuilder_Post(t *testing.T) {
	tests := []struct {
		name        string
		...
		},
		{
			name: "Error",
			setupMock: func(m *mocking.MockRequestAdapter) {
				m.On("Send", mock.Anything, mock.Anything, mock.Anything, mock.Anything).Return(nil, errors.New("attach failed"))
			},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			builder := NewAttachRequestBuilderInternal(map[string]string{"baseurl": "https://example.com"}, adapter)
...
func TestAttachRequestBuilder_Post_NilBuilder(t *testing.T) {
	var builder *AttachRequestBuilder
