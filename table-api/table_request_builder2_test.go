...
func TestTableRequestBuilder2_Get(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			Title: "Successful",
			Setup: func() {
				builder.RequestAdapter.(*mocking.RequestAdapter).On("Send", context.Background(), mock.AnythingOfType("*abstractions.RequestInformation"), mock.AnythingOfType("serialization.ParsableFactory"), mock.AnythingOfType("abstractions.ErrorMappings")).Return(new(newInt.ServiceNowCollectionResponse[serialization.Parsable]), nil)
			},
			Args: []interface{}{
				context.Background(),
