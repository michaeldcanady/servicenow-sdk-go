...
#
# Idempotent: if the ruleset declared in
# .github/policies/servicenow-sdk-go-branch-protection.yml exists it is
# updated in place; otherwise it is created. Requires gh authenticated as a
# repository administrator (rulesets are an admin API). Status checks are
# optional args — pass only universally reporting checks (the four from the
# declaration); path-filtered jobs like ci.yml's test/lint would stick
# "pending" forever on docs-only PRs. GitHub only offers checks that have
# already reported on the branch, so run once with none, open the bootstrap
# PR from the runbook, then re-apply with:
#
...
RULESET_NAME="servicenow-sdk-go-branch-protection"
